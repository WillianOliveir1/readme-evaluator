"""Example script that uses ReadmeDownloader to fetch a README from GitHub.

When executed as a script (python examples/download_readme.py ...), Python's
import path (sys.path[0]) is the `examples/` directory which prevents sibling
packages like `backend` from being importable. To make the examples runnable
directly, we prepend the project root to sys.path.
"""
import argparse
import sys
import os
...
# Ensure project root (parent of this examples/ dir) is on sys.path so
# `import backend.readme_downloader` works when running this file directly.
...
def main(argv=None):
    ...
    # Always run in verbose/debug mode (equivalent to -vv)
