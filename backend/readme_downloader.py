"""Simple Readme downloader for GitHub repositories.

Provides ReadmeDownloader class which accepts a GitHub repo URL and downloads
the repository README (using the GitHub API with fallback to raw URLs).

Usage:
    from readme_downloader import ReadmeDownloader
    downloader = ReadmeDownloader()
    path = downloader.download('https://github.com/psf/requests')
"""
...
class ReadmeDownloader:
    ...
    Methods
    - download(repo_url, dest_path=None) -> str: downloads README and returns saved path
    ...
    def _parse_repo(self, url: str) -> Tuple[str, str, Optional[str]]:
        """Extract (owner, repo, branch_opt) from GitHub repo URL forms.

        Returns branch_opt if the URL contains /tree/<branch> or /blob/<branch>/...
        """
        ...
        # https://github.com/owner/repo(.git)? optionally followed by /tree/branch or /blob/branch/...
        m = re.match(
    ...
    def _get_readme_api(self, owner: str, repo: str) -> Optional[Tuple[str, bytes]]:
        """Use GitHub API to fetch the repository README. Returns (filename, content_bytes) or None."""
    ...
    def _get_tree(self, owner: str, repo: str, branch: str) -> Optional[list]:
        """Fetch the git tree for the repository branch recursively.

        Returns a list of tree entries or None on failure.
        """
    ...
    def _find_readme_in_tree(self, tree: list) -> Optional[str]:
        """Given a git tree list, find a README path (case-insensitive)."""
        for entry in tree:
            if entry.get("type") != "blob":
                continue
            path = entry.get("path", "")
            name = os.path.basename(path)
            # match readme, readme.md, README.rst, etc.
    ...
    def _get_content_by_path(self, owner: str, repo: str, path: str, ref: Optional[str]) -> Optional[Tuple[str, bytes]]:
        """Fetch a file content by path using the contents API (returns name, bytes)."""
    ...
    def _try_raw_fallback(self, owner: str, repo: str, branch: Optional[str]) -> Optional[Tuple[str, bytes]]:
        """Try several common README filenames using the raw.githubusercontent.com endpoint (kept as fallback)."""
        if branch is None:
            branch = "main"
        candidates = [
            "README.md",
    ...
    def download(self, repo_url: str, dest_path: Optional[str] = None, prefer_api: bool = True, branch: Optional[str] = None) -> str:
        """Download README from repo_url and save to dest_path (or auto path).

        Returns the path to the saved file.
        Raises ValueError or requests exceptions on failure.
        """
        ...
        # Determine branch preference: explicit arg -> parsed from URL -> default_branch
        ...
        # Attempt 1: if we have a branch, use the git tree to find README path
        if branch_to_use:
            try:
        ...
        # Attempt 2: fallback to the simple readme API endpoint
        ...
        # Attempt 3: final fallback to raw URLs trying common names
        if not content:
            try:
        ...
        if dest_path is None:
            # create filename like owner-repo-README.md
